...
static constexpr size_t MAX_PARSER_INPUT_SIZE =
    static_cast<size_t>(std::numeric_limits<int>::max());

/**
 * Callback function for <a
 *href="http://curl.haxx.se/libcurl/c/CURLOPT_WRITEFUNCTION.html">CURLOPT_WRITEFUNCTION</a> which
 *incrementally parses the data with LibJSON.
 *
 ...
 * @throw std::out_of_range If the amount of bytes to read (i.e. size * nmemb) overflows the
 *parameter type for the JSON parser input.
 **/
static size_t parseJson(char* readBuffer, size_t size, size_t nmemb, void* pUserData)
{
	size_t result(0);
	if (pUserData)
	{
		json::TokenerParser* const pTokenerParser = static_cast<json::TokenerParser*>(pUserData);
...
ForecastServiceClient::ForecastServiceClient(curl::CallbackClient& curlClient,
                                             math::MeasurementSystem measurementUnits) noexcept
    : curlClient(curlClient), measurementUnits(measurementUnits)
{
	// ctor
}
...
std::unique_ptr<forecast_io::Forecast> ForecastServiceClient::get(const char* url)
{
	...
	const CURLcode responseCode = curlClient.read(url, parseJson, &tokenerParser);
