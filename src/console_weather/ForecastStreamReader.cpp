...
ForecastStreamReader::ForecastStreamReader(math::MeasurementSystem measurementUnits,
                                           int readBufferSize) noexcept
    : measurementUnits(measurementUnits), pBuffer(new char[static_cast<size_t>(readBufferSize)]),
...
std::unique_ptr<forecast_io::Forecast> ForecastStreamReader::read(std::istream& input)
{
	...
	const std::streamsize bytesRead = tokenerParser.parse(input, pBuffer.get(), readBufferSize);
