...
void FlagsConsoleWriter::write(const forecast_io::Flags& value, std::ostream& output)
{
	...
	static const forecast_io::MeasurementSystemNameArray& measurementNames =
	    *forecast_io::MeasurementSystemDefaultNameArrayHolder::getInstance();
	const std::string measurementName = measurementNames[static_cast<size_t>(value.getUnits())];
	output << measurementName << '\n';

	output << "Dark Sky service is available: ";
	const std::string darkskyAvailableStr = common::to_string(!value.isDarkskyUnavailable());
	...
	const std::unordered_multimap<std::string, std::string>& stations = value.getStations();
